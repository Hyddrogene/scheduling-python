...
public class Forbidden_Teachers extends ConstraintModel {
	ArrayList<Integer> variables;

	public Forbidden_Teachers(ConstraintUTP cutp) {
		super(cutp);
	}//FinMethod
	...
	public int evaluate(SolutionUTP ga) {
		int room = cutp.getParameters()[0];
			int penalties = 0;
			if(this.cutp.getSessions().size() == 1) {
				for(int j = 0 ;j < this.cutp.getSessions().get(0).size()-1 ;j++) {
						if( ga.x_slot.get(this.cutp.getSessions().get(0).get(j)-1) == room ) {
							penalties += 1;
						}
				}
			}
				...
				for(int i = 0 ;i < this.cutp.getSessions().size()-1 ;i++) {
					for(int j = 0 ;j < this.cutp.getSessions().get(i).size() ;j++) {
						for(int k = 0 ;k < this.cutp.getSessions().get((i+1)).size() ;k++) {
							if( ga.x_slot.get(this.cutp.getSessions().get(i).get(j)-1) == room ) {
								penalties += 1;
							}
						}
					}
