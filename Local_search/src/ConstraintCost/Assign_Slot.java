...
public class Assign_Slot extends ConstraintModel {
	ArrayList<Integer> variables;
	InstanceUTPArray utp;

	public Assign_Slot(ConstraintUTP cutp,InstanceUTPArray utp) {
		super(cutp);
	}//FinMethod
	...
	public int evaluate(SolutionUTP ga) {
		...
		if(this.cutp.getSessions().size() == 1) {
			for(int i = 0; i < cutp.getSessions().get(0).size() ;i++) {
				if(ga.x_slot.get(cutp.getSessions().get(0).get(i)-1) != value  ) {
					penalties++;
				}
			}
		}
