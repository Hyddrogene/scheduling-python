...
public class No_Overlap3 extends ConstraintModel {
	ArrayList<Integer> variables;
	InstanceUTPArray utp;
	ArrayList<Integer> sessions;
	int element;

	public No_Overlap3(ConstraintUTP cutp,InstanceUTPArray utp,ArrayList<Integer> sessions,int element) {
		super(cutp);
	}//FinMethod
			...
			for(int i = 0 ;i < sessions.size()-1 ;i++) {
				for(int j = 0 ;j < sessions.size()-1 ;j++) {
					if( sessions.get(i) != sessions.get(j) ) {
						if((ga.x_teachers.get(sessions.get(i)) == element && ga.x_teachers.get(sessions.get(i)) == element)) {
